...
import { Badge, Button, Input, Textarea } from '@study-assistant/ui';
...
export function AdminUserDetailDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/45 backdrop-blur-sm">
      <div className="h-full w-full max-w-2xl overflow-y-auto border-l border-border/40 bg-background p-6 shadow-soft-xl sm:p-8">
        <div className="flex items-start justify-between gap-4 border-b border-border/40 pb-6">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {detail?.flags.slice(0, 3).map((flag) => (
                <Badge key={flag.id} tone="neutral">
                  {flag.flag}
                </Badge>
              ))}
              {detail && detail.flags.length > 3 ? (
                <Badge tone="neutral">+{detail.flags.length - 3} more</Badge>
              ) : null}
            </div>
            ...
            {detail ? (
              <>
                <p className="text-xs text-muted-foreground">
                  Joined {detail.user.joinedAtFull} - Last active {detail.user.lastActiveLabel}
                </p>
                {detail.user.statusReason ? (
                  <p className="text-xs text-muted-foreground">
                    Last moderation reason: {detail.user.statusReason}
                  </p>
                ) : null}
                ...
                {detail.user.packageName ? (
                  <p className="text-xs text-muted-foreground">
                    Current package: {detail.user.packageName}
                    {detail.user.paymentStatus ? ` - ${detail.user.paymentStatus}` : ''}
                  </p>
                ) : null}
                {detail.user.nextCreditExpiryAt && detail.user.expiringCreditSeconds > 0 ? (
                  <p className="text-xs text-muted-foreground">
                    Next credit expiry: {detail.user.nextCreditExpiryAt} for {detail.user.expiringCreditSeconds / 60} minute(s) still at risk
                  </p>
                ) : null}
              </>
            ) : null}
        ...
        {loading ? (
          ...
          <div className="space-y-6 pt-6">
            ...
            >
              ...
              <div className="flex items-center justify-between gap-3 rounded-xl border border-border/40 bg-background/60 p-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Last updated {detail.access.updatedAt ? new Date(detail.access.updatedAt).toLocaleString() : 'not yet saved'}
                  </p>
                </div>
                ...
                {detail.notes.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {detail.notes.map((note) => (
                      <div key={note.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
                        <p className="text-sm text-foreground">{note.note}</p>
                        <p className="mt-2 text-xs text-muted-foreground">
                          {note.createdByName} - {note.createdByEmail} - {note.createdAt}
                        </p>
                      </div>
                    ))}
                  </div>
              ...
              {detail.transactions.length === 0 ? (
                ...
                <div className="space-y-3">
                  {detail.transactions.map((transaction) => (
                    <div key={transaction.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
                      ...
                      <p className="mt-2 text-sm text-foreground">{transaction.description}</p>
                      <p className="mt-2 text-xs text-muted-foreground">
              ...
              {detail.walletGrants.length === 0 ? (
                ...
                <div className="space-y-3">
                  {detail.walletGrants.map((grant) => (
                    <div key={grant.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <Badge
                            tone={
                      ...
                      <p className="mt-2 text-sm text-foreground">{grant.description}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {detail.sessions.length === 0 ? (
                ...
                <div className="space-y-3">
                  {detail.sessions.map((session) => (
                    <div key={session.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                      ...
                      <p className="mt-2 text-xs text-muted-foreground">
                        {session.startedAt} - {session.creditsUsed} - {session.analyzeCount} analyze
                      </p>
                    </div>
                  ))}
                </div>
                ...
                {detail.payments.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {detail.payments.map((payment) => (
                      <div key={payment.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                        ...
                        <p className="mt-2 text-sm text-foreground">{payment.amount}</p>
                ...
                {detail.devices.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {detail.devices.map((device) => {
                      ...
                      return (
                        <div key={device.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
            ...
            <DetailSection
              title="Recent Audit"
              description="Latest admin-visible changes attached to this account or wallet."
            >
              {detail.auditLogs.length === 0 ? (
                ...
                <div className="space-y-3">
                  {detail.auditLogs.map((log) => (
                    <div key={log.id} className="rounded-xl border border-border/40 bg-background/70 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-2">
