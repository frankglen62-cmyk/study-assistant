...
import { assertDistributedRateLimit, RL_AUTH_SENSITIVE } from '@/lib/security/rate-limit';
...
export async function POST(request: Request) {
  ...
  try {
    await assertDistributedRateLimit(`otp-send:ip:${ipAddress ?? 'unknown'}`, RL_AUTH_SENSITIVE);
    ...
    await assertDistributedRateLimit(`otp-send:user:${user.id}`, RL_AUTH_SENSITIVE);
