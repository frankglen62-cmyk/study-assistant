...
const TARGETS = [
  {
    name: 'Multimedia Sound and Video',
    slug: 'cs6317',
    course_code: 'CS6317',
    expectedCount: 57,
    url: 'https://www.answerscrib.com/subject/audio-and-video-production'
  }
];
...
async function scrapeSubject(config, browser, supabase, adminId) {
  ...
  if (uniqueQa.length > 0) {
    ...
    if (subErr || !subject) {
      ...
      if (createErr) {
        console.error('Failed to create subject with course code', createErr);
        const { data: newSubFallback, error: fallbackErr } = await supabase.from('subjects').insert({
          name: config.name,
          slug: config.slug,
          is_active: true
        }).select('id').single();
        if (fallbackErr) throw fallbackErr;
        subject = newSubFallback;
      } else {
        subject = newSub;
      }
    } else {
      // Update existing subject to add course code if it exists
       await supabase.from('subjects').update({ course_code: config.course_code, name: config.name }).eq('id', subject.id);
    }
